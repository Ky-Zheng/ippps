...
export default function Home() {
  return (
    <div className="grid grid-rows-3 gap-3 h-screen bg-black">
      {data.map((i)=>(
        <Card id = {i.id} title={i.title} artist={i.artist} date ={i.date} price={i.price} dimension_h={i.dimension_h} dimensions_l={i.dimensions_l} imageurl={i.imageurl}/>
      ))}
    </div>
  )
}
